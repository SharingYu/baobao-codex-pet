...
> 版本：v0.1｜日期：2026-07-15｜工作名：真实宠物桌面伙伴（正式名称待定）
...
首版交付形态：运行器免费，通用宠物可立即试玩；“我的宠物”采用付费试运营，用户先获得静态预览，确认后异步生成完整动画。早期允许后台人工质检，但对用户保持统一流程。目标交付时间不超过 24 小时；自动化稳定后再缩短到分钟级。

核心用户：养宠人、异地或独居用户、长期使用电脑的学生与上班族，以及希望纪念或长期保存宠物形象的人。

用户任务：

- 我想在工作时看到“我自己的宠物”，而不是一只通用角色。
...
```mermaid
flowchart LR
    A[安装并启动] --> B{先试玩还是创建自己的宠物}
    B -->|试玩| C[选择通用宠物]
    B -->|创建| D[上传 5 至 10 张照片]
    D --> Q{照片质检是否通过}
    Q -->|否| D1[说明问题并替换照片]
    D1 --> D
    Q -->|是| E[填写名字 性格 偏好]
...
任一宠物首次成功进入桌面后，用户必须在 3 分钟内完成摸头一次、投喂一次、玩玩具一次，并知道如何一键安静或退出；自定义宠物的异步等待时间不计入这 3 分钟。
...
- 用户可从通用宠物随时进入“创建我的宠物”，原来的试用进度不会丢失。
...
| 模块 | 需求 |
|---|---|
| 宠物创建 | 通用宠物立即试玩；照片本地质检；静态预览；一次免费重试；付费后异步生成；早期后台人工质检；失败修复或退款 |
| 基础生命感 | 走、跑、停留、观察、趴下、睡觉、伸懒腰、被抱起、被摸头 |
| 投喂 | 3 种食物；拖拽投喂；嗅闻、接受、挑食、抢食等差异反应；宠物无人照顾时会自行进食 |
| 玩具 | 逗猫棒、小球、纸箱；每次 20 秒至 2 分钟；宠物可主动结束或走神 |
| 三层互动 | 后台陪伴、5 至 20 秒快速互动、30 至 120 秒主动玩耍 |
| 个性学习 | 用户填写的是初始偏好；系统记录互动中发现的行为偏好；同一刺激至少有 3 组反应；用户可纠正或重置学习结果 |
| 羁绊相册 | 自动记录“第一次”和偏好发现；解锁行为而非堆数值等级 |
| 桌面控制 | 拖动、缩放、置顶层级、开机启动、托盘菜单、全局快捷键、一键安静、暂时收起 |
| 工作保护 | 全屏时自动安静；会议模式首版为手动快捷键；不长期遮挡点击区域；P0 不申请文件读写权限 |
| 分享 | 一键保存透明 PNG、短 GIF 或回忆卡，默认不带私密桌面内容 |
| 存档与隐私 | 本地存档；素材删除入口；崩溃后恢复状态；明确的权限说明 |

P0 分三个可独立验收的里程碑：

1. **运行器 Alpha**：通用宠物、基础状态机、摸头/拖动、安静与退出、离线存档、性能基线。
2. **陪伴 Beta**：三种食物、三种玩具、偏好学习、羁绊相册、分享导出和完整首次引导。
3. **公开 MVP**：照片质检、静态预览、支付、异步动画生成、人工质检、返工/退款和数据删除闭环。

### P1：验证留存后扩展

- 多宠物同屏，以及争玩具、分享食物、一起睡觉等组合行为。
- 激光点、捉迷藏、键盘观察和窗口边缘互动。
...
- 全屏检测仅使用操作系统窗口状态；P0 不申请麦克风、摄像头、日历和文件系统权限。
...
- 真实宠物自动生成：一次性付费。
- 人工精修与复杂饰品：高级一次性定制。
- 玩具、动作、房间和季节主题：永久 DLC。
- 订阅仅用于云同步、家庭共享和生成额度，不锁住基础陪伴。
- 不出售续命、体力、补签、免饿或随机抽卡。

免费到付费路径：用户先用通用宠物完成首次互动，在“创建我的宠物”中免费获得一张带水印静态预览；确认形象后一次性付款生成完整动画。交付与预览明显不符时提供一次返工，仍不通过则退款。

## 7. 首轮成功指标

- 通用宠物可见时间：安装完成后 60 秒内进入桌面，成功率不低于 95%。
