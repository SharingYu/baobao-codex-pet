...
> 版本：v0.2｜日期：2026-07-16｜工作名：Pet Desktop Companion
...
flowchart LR
    ...
    U -->|原路退款| D
    C --> G[宠物进入桌面]
    G --> H[导入或购买 .itempack；完成摸头 投喂 玩具 安静模式]
    H --> I[自主走动 观察 睡觉]
    I --> J{用户是否回应邀请}
    J -->|忽略| I
...
| 模块 | 需求 |
|---|---|
| 内容包与安装 | 运行器不内置宠物或道具；验证并导入独立 `.petpack` / `.itempack`；导入失败不改变现有内容；可卸载、备份、重新导入 |
| 宠物定制 | 照片本地质检；静态预览；一次免费重试；付费后异步生成；早期后台人工质检；失败修复或退款 |
| 基础生命感 | 走、跑、停留、观察、趴下、睡觉、伸懒腰、被抱起、被摸头 |
| 投喂 | 食物来自独立道具包；拖拽投喂；嗅闻、接受、挑食、抢食等差异反应；宠物无人照顾时会自行进食 |
| 玩具 | 玩具来自独立道具包；球、逗猫棒、猫窝等行为；每次 20 秒至 2 分钟；宠物可主动结束或走神 |
...
| 桌面控制 | 拖动、缩放、置顶层级、工作区边缘跑跳/坠落场景、托盘菜单、全局快捷键、一键安静、暂时收起 |
...
1. **运行器 Alpha**：空壳运行器、宠物包/道具包导入、基础状态机、摸头/拖动、工作区边缘跑跳、安静与退出、离线存档、性能基线。
...
- 前台窗口边框轨道、窗口切换后的跳跃路线，以及在用户明确开启屏幕理解后对页面视觉区域做只读互动。
...
- P0 空闲时优先待在屏幕工作区边缘或用户指定的小窝；前台窗口顶部轨道属于 P1 的可选系统适配能力。
